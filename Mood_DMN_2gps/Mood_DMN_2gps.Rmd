...
## 				FIND Var Names				
<!-- ```{r var_Names, eval=FALSE, echo=FALSE, message=FALSE, comment = ""}
#	LIST ALL THE VARIABLES IN THE DATA SET
...
dat1$sex.factor
is.factor(dat1$sex.factor)
			
dat1$v1mood.factor = factor(dat1$visit1condition, levels = c("1", "2"))  
levels(dat1$v1mood.factor) = c("PosV1", "NegV1", na.rm = FALSE)
dat1$v1mood.factor

dat1$v2mood.factor = factor(dat1$visit2condition, levels = c("1", "2")) 
levels(dat1$v2mood.factor) = c("PosV2", "NegV2", na.rm = FALSE)
dat1$v2mood.factor

			
dat1$v1.mood = factor(dat1$visit1condition, levels = c("1", "2"))  
levels(dat1$v1.mood) = c("pos", "neg", na.rm = TRUE)
dat1$v1.mood

dat1$v2.mood = factor(dat1$visit2condition, levels = c("1", "2")) 
levels(dat1$v2.mood) = c("pos", "neg", na.rm = FALSE)
dat1$v2.mood
...
dat1$isi_total <- as.numeric(dat1$isi_total)
...
dat1 <- dat1 %>% 
	filter(ID != "IP006", group.factor != "FM")

dat1 %>% count(group.factor)
str(dat1$group.factor)
...
## Subsets
Create a small data sets (dat.sm) that includes only the variables of interest.
...
dat2 <- dat1 %>% dplyr::select("ID", "group.factor",
	...
	filter(ID != "IP006") # subject not in any group
...
##				Mood Vars
```{r Moodsubsets, eval=TRUE, echo=TRUE, message=FALSE, comment = ""}
...
twoGpMood <- dat2 %>% dplyr::select("ID", "group.factor")
# view(twoGpMood)

twoGpMood <- left_join(twoGpMood, pos_dat) %>% 
	dplyr::select(-mood)

# head(twoGpMood)

twoGpMood <- full_join(twoGpMood, neg_dat) %>% 
	dplyr::select(-mood)
# head(twoGpMood)

names(twoGpMood)[names(twoGpMood) == 'group.factor'] <- 'group'
# head(twoGpMood)
#view(full_dat)

######				COMPUTE CHANGE SCORES				######
twoGpMood <- mutate(twoGpMood,
				   pos_chng = pos_post - pos_pre,
				   neg_chng = neg_post - neg_pre
				   )

twoGpMood <- arrange(twoGpMood, group)


# head(twoGpMood)

#		DROP FM GROUP
# twoGpMood <- full_dat %>% filter(full_dat$group != "FM") 
...
```{r QA-DATA, eval=FALSE, echo=TRUE, comment=""}
...
## 		Demographics
### 2 Group age & sex
```{r Demographics, eval=TRUE, echo=TRUE}
...
# > group_by_age
# # A tibble: 4 x 4
# # Groups:   group.factor [2]
#   group.factor sex.factor MeanAge ageSD
#   <fct>        <fct>        <dbl> <dbl>
# 1 HC           Male          25.0  3.08
# 2 HC           Female        24.7  4.37
# 3 CLBP         Male          27.4  6.21
# 4 CLBP         Female        36.0 13.7 
...
Descriptives for age and sex are done as of 31/12/2020
...
# group: HC
#               vars  n mean   sd   se
# group.factor*    1 38 1.00 0.00 0.00
# slpQual          2 35 0.77 0.65 0.11
# slpLat           3 38 0.79 0.78 0.13
# slpDur           4 35 0.69 0.72 0.12
# slpEff           5 35 1.20 1.37 0.23
# slpDayFcn        6 38 0.24 0.43 0.07
# psqi_Global      7 38 4.63 3.17 0.51
# ess_total        8 35 6.09 2.73 0.46
# bdi_total        9 35 3.11 4.08 0.69
# mcgill_total    10 38 1.97 7.07 1.15
# ------------------------------------------------------------------------------ 
# group: CLBP
#               vars  n  mean    sd   se
# group.factor*    1 79  2.00  0.00 0.00
# slpQual          2 63  1.21  0.79 0.10
# slpLat           3 79  1.00  0.88 0.10
# slpDur           4 63  0.86  0.72 0.09
# slpEff           5 63  1.35  1.31 0.16
# slpDayFcn        6 79  0.18  0.38 0.04
# psqi_Global      7 79  5.15  3.64 0.41
# ess_total        8 63  6.84  3.50 0.44
# bdi_total        9 64  9.84  7.84 0.98
# mcgill_total    10 79 19.80 16.74 1.88
...
###		Post Hoc - Not needed for two groups
```{r PostHoc-Scaled, eval=FALSE, echo=TRUE, message=FALSE}
...
##		Mood t-tests
```{r Mood_ttest, eval=TRUE, echo=FALSE, message=FALSE, comment = ""}

#	T-test list	####
#		1.	(data = twoGpMood) visit1moodpre vs. visit1moodpost
#		2.	(data = twoGpMood) visit1moodpre by group.factor
#		3.	(data = twoGpMood) visit1moodpost by group.factor
#		4.	(data = twoGpMood) visit1moodchange by group.factor
#		5.	(data = twoGpMood, filter = HC) visit1moodpre vs. visit1moodpost
#		6.	(data = v1Pos1, filter = FM) visit1moodpre vs. visit1moodpost



#		1. t-test pos_pre vs pos_post	####
#	1. T-test
t.test(twoGpMood$pos_pre, twoGpMood$pos_post,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#		RESULT (sig): Across groups positive mood significantly improved from pre to post

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$pos_pre and twoGpMood$pos_post
# t = -6.0321, df = 171, p-value = 9.733e-09
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -2.751895 -1.394896
# sample estimates:
...
#		2. t-test pos_pre by group		####
#	2. T-test (data = twoGpMood)
t.test(twoGpMood$pos_pre ~ twoGpMood$group,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#		RESULT (NS): HC = CLBP pre positive mood at baseline

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$pos_pre by twoGpMood$group
# t = 1.5415, df = 58.647, p-value = 0.1286
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -0.2355306  1.8148409
# sample estimates:
...
#		3. t-test pos_post by group	####
#	3. T-test (data = twoGpMood)
t.test(twoGpMood$pos_post ~ twoGpMood$group,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#			Results (NS) No significant difference in positive mood post by group

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$pos_post by twoGpMood$group
# t = 1.3933, df = 76.319, p-value = 0.1676
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -0.2745561  1.5533439
# sample estimates:
...
#		4. t-test pos_chng by group	####
#	4. T-test 	(data = twoGpMood)
t.test(twoGpMood$pos_chng ~ twoGpMood$group,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#			Results (NS) No significant difference in positive mood change by group

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$pos_chng by twoGpMood$group
# t = -0.65296, df = 47.799, p-value = 0.5169
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -0.9889918  0.5041433
# sample estimates:
...
#	5. T-test (data = twoGpMood)
t.test(twoGpMood$neg_pre, twoGpMood$neg_post,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#			Results (Sig) Significant pre vs post mood change (manipulation worked)

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$neg_pre and twoGpMood$neg_post
# t = -9.7996, df = 172.5, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -3.636258 -2.417025
# sample estimates:
...
#	6. T-test (data = twoGpMood)
t.test(twoGpMood$neg_pre ~ twoGpMood$group,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#			Results (NS) No significant difference in negative mood pre by group

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$neg_pre by twoGpMood$group
# t = -1.5468, df = 78.598, p-value = 0.1259
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -1.387429  0.174067
# sample estimates:
...
#	7. T-test (data = twoGpMood)
t.test(twoGpMood$neg_post ~ twoGpMood$group,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#			Results (Sig) A significant difference in negative mood post by group

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$neg_post by twoGpMood$group
# t = -2.1012, df = 74.157, p-value = 0.03903
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -1.8526016 -0.0491841
# sample estimates:
...
#	8. T-test 	(data = twoGpMood)
t.test(twoGpMood$neg_chng ~ twoGpMood$group,
	   mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
#			Results (NS) No significant difference in negative mood change by group

# 	Welch Two Sample t-test
# 
# data:  twoGpMood$neg_chng by twoGpMood$group
# t = -0.88449, df = 83.091, p-value = 0.379
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -1.3922838  0.5351409
# sample estimates:
...
#		CHECK LATER ANALYSES
```{r Mood_Analyses, eval=FALSE, echo=FALSE, message=FALSE, comment = ""}


##			This contains code to and from various forums
#	May 3, 2020: Issues with summary functions for bar charts
zData <- head(dput(dat2),10)
head(dput(dat2),10)

colnames(dat2)
view(dat2)

# zdataFactors2 <- select_if(dat2, is.factor)
# colnames(zdataFactors2)
# zdataFactors

#		Convert to long format
dat2.melt <- melt(dat2, id.vars = c("ID", "group.factor", "v1.mood", "v2.mood")) 
colnames(dat2.melt)

dat3.melt <- melt(dat2, id.vars = c("ID", "group.factor")) 
colnames(dat3.melt)

names(dat2.melt)[names(dat2.melt)=="variable"] <- "cond"
names(dat2.melt)[names(dat2.melt)=="value"] <- "score"

# 
# describeBy(dat2.melt,
# 		   group = "group.factor",
# 		   interp = FALSE,
# 		   skew = FALSE,
# 		   ranges = FALSE,
...
dat2.means.melt <- ddply(dat2.melt, c("group.factor", "cond", "v1.mood"),
					summarise,
...
dat2.means.melt <- ddply(dat2.melt, c("cond", "v1.mood", "v2.mood"),
					summarise,
					mean = mean(score, na.rm = TRUE),
					sd = sd(score, na.rm = TRUE)
					)

dat2.means.melt <- ddply(dat2.melt, c("group.factor", "v1.mood"),
					summarise,
					mean = mean(score, na.rm = TRUE),
					sd = sd(score, na.rm = TRUE))
...
#### Extra 1 (Main effect of group)
```{r ANOVA-scaleddata2, eval=FALSE, echo=FALSE, message=FALSE}
# construct anova model for each column of dat1[, my_outcome_variables]
...
#### Extra 2 POSTHOC
```{r PostHoc-Scaled2, eval=FALSE, echo=FALSE, message=FALSE}
GetTukeys <- function(m) m$tukey
Tukeys  <- purrr::map(model_summaries, GetTukeys)  %>%
	purrr::map_dfr(broom::tidy, .id = "Name")
# head(Tukeys)
